...
# Class: Agent for managing open and closing trades
class BotAgent:

    """
        Primary func of bot, opening and checking order status
    """
    ...
    def __init__(
        ...
        # Initialize output variable, pair status options failed, live, close, error
        self.order_dict = {
            "market_1": market_1,
            "market_2": market_2,
    ...
    def check_order_status_by_id(self, order_id):
        ...
        # Guard: if the order got cancelled, move to next pair
        if order_status == "CANCELED":
            print(f"{self.market_1} vs {self.market_2} - Order cancelled...")
            self.order_dict["pair_status"] = "FAILED"
            return "failed"
        
        # Guard: order not filled, wait until order expiration
    ...
    def open_trades(self):
        ...
        # Guard: Abort if the order failed
        if order_status_m1 != "live":
            self.order_dict["pair_status"] = "ERROR"
            self.order_dict["comments"] = f"{self.market_1} failed to fill"
            return self.order_dict
        
        #print status opening second order, creat order for second pair
        print("---")
        ...
        if order_status_m2 != "live":
            self.order_dict["pair_status"] = "ERROR"
            self.order_dict["comments"] = f"{self.market_2} failed to fill" #market_1??
            ...
            try: 
                ...
                # Ensure order is live before continuing
                time.sleep(2)
                order_status_close_order = check_order_status(self.client, close_order["order"]["id"]) #need a backup, message sent to user, if bot is not able to close first trade
                if order_status_close_order != "FILLED":
                    print("ABORT PROGRAM")
                    print("Unexpected Error")
                    print(order_status_close_order)

                    # Create message here, to message user that they need to check up on it!!!
