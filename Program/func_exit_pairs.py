...
def manage_trade_exits(client):
    ...
    Baed upon criteria set in constants
    """
    ...
    #Guard; Exits if no open position in sfile
    ...
    #Chekc all saved positions match order record, exit trade according to any exit rules
    for position in open_positions_dict:

        #Initialize close trigger
        is_close = False

        #Extract position matching info from file - market_1
        ...
        #debugging
        # print(f"Checking positions for {position_market_m1} and {position_market_m2}")
        # print(f"Order markets: {order_market_m1} and {order_market_m2}")
        # print(f"Check conditions - M1: {check_m1}, M2: {check_m2}, Live: {check_live}")

        #Guaard: If not all match exit with error
        ...
        if CLOSE_AT_ZSCORE_CROSS:
            ...
            #print(series_1[-1], series_2[-1]) this doesnt work, first pair gives error so check it
        ...
        ###
        #Add my own close conditions
        ###

        #Close positions if triggered 
        if is_close: #Change not when actually running
            ...
            try:
                ...
                print(f"Closing positons for {position_market_m1}")
                ...
                print(f"Exit fail for {position_market_m1} with {position_market_m2}")
